#ifndef STINGRAY_TOOLKIT_CHUNKEDBYTEDATA_H
#define STINGRAY_TOOLKIT_CHUNKEDBYTEDATA_H

#include <stingray/toolkit/ByteData.h>
#include <stingray/toolkit/IEnumerable.h>


namespace stingray
{

	TOOLKIT_DECLARE_ENUMERABLE(ByteArray);
	TOOLKIT_DECLARE_ENUMERABLE(ByteData);
	TOOLKIT_DECLARE_ENUMERABLE(ConstByteArray);
	TOOLKIT_DECLARE_ENUMERABLE(ConstByteData);

}


#endif
