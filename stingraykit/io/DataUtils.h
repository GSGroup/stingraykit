...
namespace stingray
{
	...
	class DataTaker : public virtual IDataSource
	{
		...
		public:
			Consumer(IDataConsumer& consumer, size_t count) : _consumer(consumer), _leftToTake(count) { }

			size_t Process(ConstByteData data, const ICancellationToken& token) override
			{
				const size_t size = std::min(data.size(), _leftToTake);
				if (!size)
					return 0;
	...
	public:
		...
		{
			Consumer taker(consumer, _leftToTake);
