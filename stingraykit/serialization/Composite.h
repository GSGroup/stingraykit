...
#define STINGRAYKIT_DECLARE_COMPOSITE_MEMBER_IMPL_3(MemberType, MemberName, SerializationKey) \
		(MemberType, MemberName, SerializationKey, DefaultSerializationTag, DefaultSerializationTag)

#define STINGRAYKIT_DECLARE_COMPOSITE_MEMBER_IMPL_4(MemberType, MemberName, SerializationKey, SerializationTag) \
		(MemberType, MemberName, SerializationKey, SerializationTag, SerializationTag)
...
#define DETAIL_MEMBER_TYPE_NTH_METHOD(N, ...)			STINGRAYKIT_NTH_ELEMENT(0, MK_PARAM STINGRAYKIT_NTH_ELEMENT(N, __VA_ARGS__))
#define DETAIL_MEMBER_NAME_NTH_METHOD(N, ...)			STINGRAYKIT_NTH_ELEMENT(1, MK_PARAM STINGRAYKIT_NTH_ELEMENT(N, __VA_ARGS__))
#define DETAIL_SERIALIZATION_KEY_NTH_METHOD(N, ...)		STINGRAYKIT_NTH_ELEMENT(2, MK_PARAM STINGRAYKIT_NTH_ELEMENT(N, __VA_ARGS__))
#define DETAIL_SERIALIZATION_TAG_NTH_METHOD(N, ...)		STINGRAYKIT_NTH_ELEMENT(3, MK_PARAM STINGRAYKIT_NTH_ELEMENT(N, __VA_ARGS__))
#define DETAIL_DESERIALIZATION_TAG_NTH_METHOD(N, ...)	STINGRAYKIT_NTH_ELEMENT(4, MK_PARAM STINGRAYKIT_NTH_ELEMENT(N, __VA_ARGS__))
...
#define DETAIL_COMPOSITE_CTOR(ClassName, ...) \
		public: ClassName(STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_CTOR_PARAM, __VA_ARGS__)) : \
		STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_CTOR_MEMBER_INITIALIZATION, __VA_ARGS__) { } \
		ClassName() { }


#define DETAIL_COMPOSITE_MEMBER_LIST(ClassName, ...) \
		STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_MEMBER, ClassName, __VA_ARGS__)
...
#define DETAIL_COMPOSITE_TOSTRING(ClassName, ...) \
		public: std::string ToString() const { return StringBuilder() % "{ " % STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_TOSTRING_IMPL, __VA_ARGS__) % " }"; }

#define DETAIL_COMPOSITE_TOSTRING_IMPL(N, ...) \
		DETAIL_ESCAPED_COMMA_IF(N) TO_STRING(DETAIL_MEMBER_NAME_NTH_METHOD(N, __VA_ARGS__)) % ": " % STINGRAYKIT_CAT(_, DETAIL_MEMBER_NAME_NTH_METHOD(N, __VA_ARGS__))
...
#define DETAIL_COMPOSITE_SERIALIZER(ClassName, ...) \
		template <typename StreamType> void Serialize(StreamType& ar) const { STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_SERIALIZER_IMPL, __VA_ARGS__) }
...
#define DETAIL_COMPOSITE_DESERIALIZER(ClassName, ...) \
		template <typename StreamType> void Deserialize(StreamType& ar) { STINGRAYKIT_REPEAT(STINGRAYKIT_NARGS(__VA_ARGS__), DETAIL_COMPOSITE_DESERIALIZER_IMPL, __VA_ARGS__) }
