...
namespace stingray {
	...
	namespace Detail
	{
		...
		struct DataPipeStage
		{
			...
			{ return stages.GetHead().Process(input, Bind(&NextStage::template Process<OutputType, Consumer>, wrap_ref(stages.GetTail()), _1, wrap_const_ref(consumer))); }
			...
			static RetType EndOfData(StagesTuple& stages, const Consumer& consumer, const EodFunc& eod)
			{
				return stages.GetHead().EndOfData(
					Bind(&NextStage::template Process<OutputType, Consumer>, wrap_ref(stages.GetTail()), wrap_const_ref(consumer), _1),
					Bind(&NextStage::template EndOfData<Consumer, EodFunc>, wrap_ref(stages.GetTail()), wrap_const_ref(consumer), wrap_const_ref(eod)));
			}
