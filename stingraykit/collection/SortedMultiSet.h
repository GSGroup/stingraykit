...
namespace stingray
{
	...
	public:
		SortedMultiSet() : _items(make_shared<SetType>())
		{ }

		SortedMultiSet(const SortedMultiSet& other) : _items(make_shared<SetType>(*other._items))
		{ }
		...
		SortedMultiSet(shared_ptr<IEnumerator<T> > enumerator) : _items(make_shared<SetType>())
		{
			STINGRAYKIT_REQUIRE_NOT_NULL(enumerator);
			Enumerable::ForEach(enumerator, bind(&SortedMultiSet::Add, this, _1));
		}

		SortedMultiSet(shared_ptr<IEnumerable<T> > enumerable) : _items(make_shared<SetType>())
		{
